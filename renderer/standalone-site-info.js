(() => {
  ...
  async function render() {
    let info = null;
    try {
      info = await window.browserAPI.getStandaloneSiteInfo?.();
    } catch (_err) {
      info = null;
    }
  ...
  window.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      // Standalone pencerede ESC ile kapatma desteği eklenebilir
    }
  });

  // Refresh when data updates
  window.browserAPI.onStandaloneSiteInfoRefresh?.(() => render());
